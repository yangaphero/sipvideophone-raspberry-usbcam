...
{
	struct audio_param_t *audioparam=AudioParam;
	//char *audio_hw, char *dest_ip, int dest_port
	
	fprintf(stderr,GREEN"[%s]:"NONE"param:audio_hw=%s,dest_ip=%s,dest_port=%d\n",__FILE__,audioparam->audio_hw,audioparam->dest_ip,audioparam->dest_port);
    ...
    //端口复用
    int flag=1;
    if( setsockopt(rtp_socket, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(int)) == -1)  {  
    ...
    local.sin_port=htons(audioparam->local_port);            ///监听端口  
    local.sin_addr.s_addr=INADDR_ANY;       ///本机  
    if(bind(rtp_socket,(struct sockaddr*)&local,sizeof(local))==-1) {
